...
- **RPR deep link** — a "View on RPR" button on the property workspace that opens **that property** inside the agent's own RPR (Realtors Property Resource) account, through their MLS sign-in. `SubAccountDoc.rprOrgId` is RPR's public per-MLS-board code (e.g. `ctconnm-n` for SmartMLS/connectMLS), set once by the admin under Settings → RPR ([sub-account-rpr-section.tsx](src/components/settings/sub-account-rpr-section.tsx)) via `POST/DELETE /api/sub-accounts/[id]/rpr-integration`. Not a credential — it's a fixed constant visible in RPR's own URLs once signed in, not stored in the secrets subcollection. The link goes through RPR's **Deep Links** endpoint (`narrpr.com/deep-link`), keyed by the listing's `mlsId` (`listingid=`) where there is one and its full address (`query=` + `searchtype=Properties`) otherwise; it falls back to the MLS-SSO home entry (`www.narrpr.com/home?cbcode={rprOrgId}`) when the property has neither. **This replaced an earlier belief that RPR could not be linked per-property at all.** That belief came from RPR resolving property pages through an internal id (`/properties/details/info/{rprInternalId}`) that we cannot obtain — true, and verified live that a fabricated id 404s — but RPR's deep-link endpoint resolves that id itself from an MLS number or address. **Verified live against a signed-in RPR session on both paths:** an MLS-number link resolved to `/properties/details/info/78456398` and an address-only link to `/properties/details/info/78501778`, each the correct property. An earlier clipboard-copy hedge (added while the destination was unconfirmed) has been removed — it clobbered whatever the agent had copied and bought nothing once the link was shown to land. Requires an address carrying city + state (a bare street line would resolve to the wrong town, so it returns null instead). `narrpr.com` is egress-blocked from CI and the build environment, so this cannot be re-checked automatically; re-verifying takes a signed-in browser and one click per path. The button self-locks (shows "Set your RPR org code in Settings" instead of a dead link) until `rprOrgId` is set. See [src/lib/rpr/link.ts](src/lib/rpr/link.ts).
...
### Facebook Messenger + Instagram DM inbox (PREVIEW — two-way)
...
- **Token auto-refresh (no re-sign-in needed):** the OAuth callback exchanges the short-lived code for a long-lived (~60 day) USER token via `exchangeForLongLivedUserToken()` (Meta's `fb_exchange_token` grant) BEFORE deriving the Page token — the Page token inherits whichever user token it was fetched with, so skipping this step would have made every connection die almost immediately. That long-lived user token is stored alongside the Page token in the secrets subcollection (`subAccounts/{id}/secrets/meta`, `MetaSecrets.userAccessToken`). The weekly `agentstack-meta-token-refresh` QStash schedule (see `lib/qstash/register-schedules.ts`) fans out to [src/lib/comms/meta-refresh.ts](src/lib/comms/meta-refresh.ts)::`refreshMetaConnection()` for every connected sub-account: it re-exchanges the stored user token for a fresh one (Meta allows this as long as the current one hasn't expired yet, which is what makes the connection effectively permanent) and re-derives a fresh Page token from it. On any failure — revoked access, the Page removed, a legacy connection made before this existed with no stored user token to refresh from — it sets `metaConfig.needsReconnect = true` rather than letting sends fail silently; the Settings card ([sub-account-meta-section.tsx](src/components/settings/sub-account-meta-section.tsx)) surfaces this as an explicit amber "this connection needs to be refreshed" banner with a Reconnect button, never a guess. A successful reconnect clears the flag automatically (the callback does a full-object `metaConfig` write, not a merge).
...
1. **(One-time, developer)** Meta app configured with `META_APP_ID`/`META_APP_SECRET`; register the OAuth redirect URI (`…/api/sub-accounts/{id}/meta/callback`) and webhook URL (`…/api/webhooks/meta`) shown on the Settings card; submit Meta App Review for the messaging **and** posting permissions; `firebase deploy --only firestore:rules`.
...
### Optional — Post-payment GitHub auto-invite

Powers the **"Get instant repo access"** button on `/thank-you`. When a buyer enters their GitHub username, the server calls the GitHub API to add them to a team in your org — exactly the same path operators use today for manual additions, just automated. Without these vars set, `/api/github/invite` returns a friendly error and buyers fall back to the manual chat path.
...
- The defence: `/api/checkout/founders/route.ts` generates a 256-bit `claimToken` at session-create time, stamps it into both `session.metadata.claimToken` and the `success_url` as `&t=...`.
- The Stripe webhook hashes the token (SHA-256) and stores **only the hash** on `purchases/{sessionId}.claimTokenHash`. The raw token never touches the database.
- `/api/github/invite` re-hashes the buyer's submitted token and constant-time-compares to the stored hash. Mismatch → 403.
- 3-attempt lock per session on top of the token gate, plus a transient 5-attempts-per-10-min rate limit.
- Race-condition safe: if the buyer clicks before the webhook lands, the endpoint returns 425 and the dialog auto-retries.

If the org has SAML/SSO enforced, you'll need to authorise the fine-grained PAT for SSO after generating it (org settings → Personal access tokens → Active tokens → Authorize). Without that, GitHub returns 403 even with correct permissions.
...
an individual agent's or brokerage's own real-estate sales website is one
such customer. A customer and the platform are not the same entity, are not
affiliated for legal purposes, and must never be blended in code, content,
tests, or documents. That includes naming a real customer in this file: the
rule is stated with generic examples on purpose.
