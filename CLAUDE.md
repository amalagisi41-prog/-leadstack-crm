...
- **RPR deep link** — a "View on RPR" button on the property workspace that opens the agent's own RPR (Realtors Property Resource) account via their MLS-SSO entry point (`narrpr.com/home?cbcode={rprOrgId}`), and copies the property address to the clipboard to paste into RPR's search box. `SubAccountDoc.rprOrgId` is RPR's public per-MLS-board code (e.g. `ctconnm-n` for SmartMLS/connectMLS), set once by the admin under Settings → RPR ([sub-account-rpr-section.tsx](src/components/settings/sub-account-rpr-section.tsx)) via `POST/DELETE /api/sub-accounts/[id]/rpr-integration`. Not a credential — it's a fixed constant visible in RPR's own URLs once signed in, not stored in the secrets subcollection. Deliberately does **not** attempt a per-property deep link: RPR resolves every property page through its own internal property id, which is unobtainable without RPR API access (off-limits under RPR's terms — no third-party data extraction); verified live that a fabricated id 404s and an address search still resolves through that same internal id rather than an intermediate linkable page. The button self-locks (shows "Set your RPR org code in Settings" instead of a dead link) until `rprOrgId` is set. See [src/lib/rpr/link.ts](src/lib/rpr/link.ts).
