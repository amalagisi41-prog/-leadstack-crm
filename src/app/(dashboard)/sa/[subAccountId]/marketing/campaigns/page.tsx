...
export default function MarketingCampaignsPage() {
  ...
  async function importListing(event: ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      const totalBytes = listingFile.size + photoFiles.reduce((total, file) => total + file.size, 0);
      if (totalBytes > MAX_UPLOAD_BYTES) {
        throw new Error("Keep the combined listing export and photos under 4 MB for this upload. Use a smaller export or fewer/compressed photos.");
      }
      ...
      toast.success(`Imported ${data.listing.address} and built drafts across all channels.`);
  ...
  return <div className="space-y-6">
    ...
    {brief && <div className="space-y-4"><div className="rounded-2xl border bg-card p-5"><div className="flex items-start gap-3"><Sparkles className="mt-0.5 h-5 w-5 text-primary" /><div><h2 className="font-semibold">{brief.brief.title}</h2><p className="mt-1 text-sm text-muted-foreground">{brief.brief.description}</p><p className="mt-2 text-xs">Boost schedule: {brief.brief.boostTier ?? "not eligible yet"}{brief.brief.daysOnMarket == null ? " · days on market unavailable" : ` · ${brief.brief.daysOnMarket} days on market`}</p></div></div>{brief.brief.dataGaps.length > 0 && <p className="mt-4 text-xs text-amber-700">Data gaps (not invented): {brief.brief.dataGaps.join(", ")}</p>}<div className="mt-4 flex flex-wrap items-center gap-3"><Button onClick={approveReadyDrafts} disabled={!isAdmin || approving}>{approving ? "Approving…" : "Approve ready drafts"}</Button>{landingPageUrl && <a className="text-sm underline" href={landingPageUrl} target="_blank" rel="noreferrer">View approved landing page</a>}</div></div><div className="rounded-2xl border bg-card p-5"><div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">LSEO &amp; SERP strategy</h2><p className="text-xs text-muted-foreground">Guidance for local discoverability using verified facts only.</p></div><span className="rounded-full bg-muted px-3 py-1 text-sm font-medium">{brief.brief.lseo.score}/100 readiness</span></div><div className="mt-4 grid gap-3 md:grid-cols-2"><div><p className="text-xs font-medium uppercase text-muted-foreground">Search preview</p><p className="mt-1 text-sm font-medium">{brief.brief.lseo.searchTitle}</p><p className="mt-1 text-xs text-muted-foreground">{brief.brief.lseo.metaDescription}</p><p className="mt-3 text-xs"><span className="font-medium">Primary query:</span> {brief.brief.lseo.primaryQuery}</p></div><div><p className="text-xs font-medium uppercase text-muted-foreground">Recommended next steps</p><ul className="mt-1 list-disc space-y-1 pl-4 text-xs text-muted-foreground">{brief.brief.lseo.recommendations.map((recommendation) => <li key={recommendation}>{recommendation}</li>)}</ul></div></div>{brief.brief.lseo.blockers.length > 0 && <div className="mt-4 rounded-lg bg-amber-50 p-3 text-xs text-amber-800"><p className="font-medium">Approval blockers</p><ul className="mt-1 list-disc space-y-1 pl-4">{brief.brief.lseo.blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul></div>}</div><div className="grid gap-3 md:grid-cols-2">{brief.brief.channels.map((draft) => <div key={draft.channel} className="rounded-xl border bg-card p-4"><div className="flex items-center justify-between"><h3 className="font-medium capitalize">{draft.channel}</h3><span className="text-xs text-muted-foreground">{draft.approval}</span></div><p className="mt-2 text-sm">{draft.body}</p>{draft.findings.length > 0 && <p className="mt-2 flex gap-1 text-xs text-amber-700"><AlertTriangle className="h-4 w-4 shrink-0" /> Review: {draft.findings.join(", ")}</p>}<p className="mt-2 text-[11px] text-muted-foreground">Status: {draft.status} · audited approval required</p></div>)}</div></div>}
  </div>;
}
