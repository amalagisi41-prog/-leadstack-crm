...
export function DomainConnect() {
  ...
  async function continueToTransfer() {
    ...
    try {
      ...
      setTransfer(data.transfer);
      window.location.assign(HOSTINGER_TRANSFER_URL);
    } catch (error) {
      toast.error(
    ...
    <div className="mx-auto max-w-5xl space-y-5">
      <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="bg-gradient-to-r from-[#173b7a] to-[#315f9d] p-6 text-white">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-blue-100">
            Choose one path. AgentStack saves your progress, keeps the current
            website live, and unlocks DNS only after the hosted site is
            verified.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <section className="rounded-2xl border bg-white p-6">
          <p className="text-xs font-semibold tracking-[0.16em] text-blue-700 uppercase">
            {situation === "switching" ? "CRM migration" : "Website transfer"}
          </p>
          <h2 className="mt-1 text-lg font-semibold">
            Save the current address before opening the provider step
          </h2>
          <p className="text-muted-foreground mt-1 text-sm">
            We store the address and status only. The current site is not copied
            into an AgentStack iframe.
          </p>
          ...
          {transfer ? (
            <div className="mt-5 rounded-xl border border-blue-200 bg-blue-50 p-4">
              ...
              <p className="mt-1 text-xs leading-5 text-blue-900">
                The provider step may leave AgentStack because providers do not
                all support embedding. Your status is saved here before you go.
              </p>
              <Button
                className="mt-3"
        ...
        <section className="rounded-2xl border bg-white p-6">
          ...
          <h2 className="mt-1 text-lg font-semibold">
            Save a domain, then start the private design
          </h2>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button variant="outline" render={<a href={HOSTINGER_NEW_URL} />}>
      ...
      <section className="rounded-2xl border bg-slate-50 p-5">
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <StatusCard
            label="DNS"
            value={hostingReady ? "Ready for review" : "Safely locked"}
            complete={hostingReady}
          />
        </div>
        {!hostingReady ? (
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
            <p className="text-xs leading-5 text-amber-950">
              No DNS change is available until hosting, SSL, and the target site
              are verified.
            </p>
            <Button
              size="sm"
              ...
              onClick={() =>
                openAskAssistant({
                  prompt:
                    "Explain my Website & Domain status in plain language and give me only the next safe action. Do not provide DNS records unless hosting and SSL are verified.",
                })
              }
            >
              Ask Zack what’s next
            </Button>
          </div>
        ) : null}
      </section>
